...
int main()
{
    ...
    ellipse.setFillColor(sf::Color(0xFF, 0xFF, 0x00));

    //Инициализируем вершины псевдо-эллипса.
    ellipse.setPointCount(pointCount);
    for (int pointNo = 0; pointNo < pointCount; ++pointNo)
    {
        float angel = float(2 * M_PI * pointNo) / float(pointCount);
        sf::Vector2f point = {
            ellipseRadius.x * std::sin(angel),
            ellipseRadius.y * std::cos(angel)};
